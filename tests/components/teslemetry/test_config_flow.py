...
def _local_client(public_key: str, state: str) -> dict[str, Any]:
    """Return one entry of a local list_authorized_clients response."""
    return {
...
def _local_clients_payload(*clients: dict[str, Any]) -> dict[str, Any]:
    """Return a local list_authorized_clients response carrying the given entries."""
    return {"response": {"clients": list(clients), "enable_line_switch_off": False}}
...
async def test_reconfigure_reads_authorized_clients_locally(
    ...
    mock_local_authorized_clients.return_value = _local_clients_payload(*clients)
...
async def test_reconfigure_local_unrecognized_state_aborts(
    ...
    subentry_id = entry.get_subentries_of_type(SUBENTRY_TYPE_ENERGY_SITE)[0].subentry_id
    mock_local_authorized_clients.side_effect = None
    mock_local_authorized_clients.return_value = _local_clients_payload(
        _local_client(PUBLIC_KEY_B64, "SOME_FUTURE_STATE")
    )
