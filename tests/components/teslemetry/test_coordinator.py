"""Tests for the Teslemetry energy history coordinator."""
...
async def test_coordinator_first_run(
    ...
    with patch(
        "homeassistant.components.teslemetry.coordinator.get_instance"
    ) as mock_get_instance:
        mock_get_instance.return_value = recorder_mock
        coordinator = TeslemetryEnergyHistoryCoordinator(
    ...
    stats = await hass.async_add_executor_job(
        ...
        None,
        {
            "teslemetry:123456_solar_energy_exported",
            "teslemetry:123456_grid_energy_imported",
            "teslemetry:123456_battery_energy_exported",
        },
        "hour",
        None,
        STATISTIC_TYPES,
...
async def test_coordinator_skips_existing_timestamps(
    ...
    assert len(stats["teslemetry:123456_solar_energy_exported"]) == 1
...
async def test_coordinator_handles_missing_timestamp(
    ...
    assert len(stats["teslemetry:123456_solar_energy_exported"]) == 1
    assert stats["teslemetry:123456_solar_energy_exported"][0].get("sum") == 2000
