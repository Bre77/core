...
async def test_climate(hass: HomeAssistant, snapshot: SnapshotAssertion) -> None:
    ...
    assert hass.states.get(entity_id) == snapshot(name=SERVICE_SET_HVAC_MODE)
    ...
    with patch(
        ...
        mock_set.assert_called_once()
    assert hass.states.get(entity_id) == snapshot(name=SERVICE_SET_TEMPERATURE)
    ...
    with patch(
        ...
        mock_set.assert_called_once()
    assert hass.states.get(entity_id) == snapshot(name=SERVICE_SET_PRESET_MODE)
