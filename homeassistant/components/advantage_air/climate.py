...
class AdvantageAirAC(AdvantageAirAcEntity, ClimateEntity):
    ...
    async def async_set_hvac_mode(self, hvac_mode: HVACMode) -> None:
        ...
        if hvac_mode == HVACMode.OFF:
            await self.set({self.ac_key: {"info": {"state": ADVANTAGE_AIR_STATE_OFF}}})
        else:
            await self.set(
                {
                    self.ac_key: {
    ...
    async def async_set_fan_mode(self, fan_mode: str) -> None:
        """Set the Fan Mode."""
        await self.set({self.ac_key: {"info": {"fan": HASS_FAN_MODES.get(fan_mode)}}})
...
class AdvantageAirZone(AdvantageAirZoneEntity, ClimateEntity):
    ...
    async def async_set_temperature(self, **kwargs: Any) -> None:
        ...
        await self.set({self.ac_key: {"zones": {self.zone_key: {"setTemp": temp}}}})
